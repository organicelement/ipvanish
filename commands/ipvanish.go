...
func Execute() {
	ZcastCmd.SetGlobalNormalizationFunc(helpers.NormalizeHugoFlagsFunc)
	AddCommands()
	utils.StopOnErr(ZcastCmd.Execute())
}

//AddCommands adds child commands to the root command HugoCmd.
func AddCommands() {
	ZcastCmd.AddCommand(serverCmd)
	ZcastCmd.AddCommand(version)
	ZcastCmd.AddCommand(config)
	ZcastCmd.AddCommand(check)
	ZcastCmd.AddCommand(benchmark)
	ZcastCmd.AddCommand(convertCmd)
	ZcastCmd.AddCommand(newCmd)
	ZcastCmd.AddCommand(listCmd)
	ZcastCmd.AddCommand(undraftCmd)
	ZcastCmd.AddCommand(genautocompleteCmd)
	ZcastCmd.AddCommand(gendocCmd)
}

//Initializes flags
func init() {
	ZcastCmd.PersistentFlags().BoolVarP(&Draft, "buildDrafts", "D", false, "include content marked as draft")
	ZcastCmd.PersistentFlags().BoolVarP(&Future, "buildFuture", "F", false, "include content with publishdate in the future")
	ZcastCmd.PersistentFlags().BoolVar(&DisableRSS, "disableRSS", false, "Do not build RSS files")
	ZcastCmd.PersistentFlags().BoolVar(&DisableSitemap, "disableSitemap", false, "Do not build Sitemap file")
	ZcastCmd.PersistentFlags().StringVarP(&Source, "source", "s", "", "filesystem path to read files relative from")
	ZcastCmd.PersistentFlags().StringVarP(&CacheDir, "cacheDir", "", "", "filesystem path to cache directory. Defaults: $TMPDIR/hugo_cache/")
	ZcastCmd.PersistentFlags().BoolVarP(&IgnoreCache, "ignoreCache", "", false, "Ignores the cache directory for reading but still writes to it")
	ZcastCmd.PersistentFlags().StringVarP(&Destination, "destination", "d", "", "filesystem path to write files to")
	ZcastCmd.PersistentFlags().StringVarP(&Theme, "theme", "t", "", "theme to use (located in /themes/THEMENAME/)")
	ZcastCmd.PersistentFlags().BoolVarP(&Verbose, "verbose", "v", false, "verbose output")
	ZcastCmd.PersistentFlags().BoolVar(&UglyURLs, "uglyURLs", false, "if true, use /filename.html instead of /filename/")
	ZcastCmd.PersistentFlags().StringVarP(&BaseURL, "baseURL", "b", "", "hostname (and path) to the root, e.g. http://spf13.com/")
	ZcastCmd.PersistentFlags().StringVar(&CfgFile, "config", "", "config file (default is path/config.yaml|json|toml)")
	ZcastCmd.PersistentFlags().StringVar(&Editor, "editor", "", "edit new content with this editor, if provided")
	ZcastCmd.PersistentFlags().BoolVar(&Logging, "log", false, "Enable Logging")
	ZcastCmd.PersistentFlags().StringVar(&LogFile, "logFile", "", "Log File path (if set, logging enabled automatically)")
	ZcastCmd.PersistentFlags().BoolVar(&VerboseLog, "verboseLog", false, "verbose logging")
	ZcastCmd.PersistentFlags().BoolVar(&nitro.AnalysisOn, "stepAnalysis", false, "display memory and timing of different steps of the program")
	ZcastCmd.PersistentFlags().BoolVar(&PluralizeListTitles, "pluralizeListTitles", true, "Pluralize titles in lists using inflect")
	ZcastCmd.PersistentFlags().BoolVar(&PreserveTaxonomyNames, "preserveTaxonomyNames", false, `Preserve taxonomy names as written ("Gérard Depardieu" vs "gerard-depardieu")`)

	ZcastCmd.Flags().BoolVarP(&BuildWatch, "watch", "w", false, "watch filesystem for changes and recreate as needed")
	ZcastCmd.Flags().BoolVarP(&NoTimes, "noTimes", "", false, "Don't sync modification time of files")
	hugoCmdV = ZcastCmd

	// for Bash autocomplete
	validConfigFilenames := []string{"json", "js", "yaml", "yml", "toml", "tml"}
	ZcastCmd.PersistentFlags().SetAnnotation("config", cobra.BashCompFilenameExt, validConfigFilenames)
	ZcastCmd.PersistentFlags().SetAnnotation("theme", cobra.BashCompSubdirsInDir, []string{"themes"})
